...
**Dedicated MCP tools:** Use `mcp__skywatch-mcp__cosharing_clusters`, `mcp__skywatch-mcp__cosharing_pairs`, `mcp__skywatch-mcp__cosharing_evolution` for structured access (these support JOINs across the three tables internally). Use `mcp__skywatch-mcp__clickhouse_query` for ad-hoc queries.
...
Use the `mcp__skywatch-mcp__clickhouse_schema` MCP tool to get column metadata:

```
Tool: mcp__skywatch-mcp__clickhouse_schema
Purpose: Return column names and types for osprey_execution_results
```

Use the `mcp__skywatch-mcp__clickhouse_query` MCP tool to execute SELECT queries:

```
Tool: mcp__skywatch-mcp__clickhouse_query
Parameters:
...
- Use the `mcp__skywatch-mcp__clickhouse_query` MCP tool to execute exploratory queries
