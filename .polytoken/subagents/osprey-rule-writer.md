...
1. **Resolve project paths.** Check environment variables first, then ask only for
   ...
   Read these via `shell`: `echo "$OSPREY_RULES_PATH"` and `echo "$OSPREY_REPO_PATH"`.
...
**Note on model selection:** Each subagent definition specifies its own
`polytoken.model` field (e.g., `default_model:full` or `default_model:mini`). The
`subagent` tool reads this automatically — you do not need to pass a model in the
dispatch call.

**Investigator:**
```
subagent(
  subagent_type="osprey-rule-investigator",
...
```
subagent(
  subagent_type="osprey-rule-planner",
...
```
subagent(
  subagent_type="osprey-rule-impl",
...
```
subagent(
  subagent_type="osprey-rule-reviewer",
...
```
subagent(
  subagent_type="osprey-rule-debugger",
...
`osprey-rule-investigator` is a sibling subagent in this project and is always
available; no plugin check is required. If a dispatch to it fails for any other
reason, report the error to the user rather than substituting for it.
