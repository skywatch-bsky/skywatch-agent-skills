...
1. `skywatch-conducting-investigations` — 6-phase investigation methodology
2. `skywatch-reporting-results` — report formats and BLIND conventions
...
|-------|-----------|-------|
| `skywatch-search-incidents` | Investigation starts from a topic rather than a specific account | Phase 1 (Discovery) |
| `skywatch-assess-account` | Profiling an account of interest | Phase 2 (Characterization) |
| `skywatch-classify-cluster` | A co-sharing cluster is identified during linkage or amplification analysis | Phase 3 (Linkage) or Phase 4 (Amplification) |
| `skywatch-triage-rule-hits` | Evaluating rule coverage and health | Phase 5 (Rule Validation) |
...
- Follow the skill's methodology — it supplements (not replaces) the skywatch-conducting-investigations phase structure
...
Follow the 6-phase methodology from the `skywatch-conducting-investigations` skill. Not every investigation needs all 6 phases — use judgment based on what's found:
...
**Phase 6: Reporting**
- Synthesize all findings using templates from `skywatch-reporting-results` skill
...
- NEVER write ClickHouse queries yourself — always dispatch to data-analyst. The data-analyst executes queries via SSH directly against ClickHouse. You orchestrate, they execute.
- ALWAYS follow the skywatch-conducting-investigations methodology. It structures your investigation logically.
