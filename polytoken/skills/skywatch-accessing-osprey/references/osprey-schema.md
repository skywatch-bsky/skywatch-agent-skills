...
There are ~600 rule and model columns. Use `DESCRIBE TABLE default.osprey_execution_results` via SSH to get the current full list, or query system.columns directly:
...
7 days. Queries beyond that window return no results. Use `mcp__skywatch-mcp__cosharing_pairs` MCP tool for structured access, or query the table directly via SSH.
...
7 days. For cluster-level data beyond 7 days, use `url_cosharing_clusters` (no TTL). Use `mcp__skywatch-mcp__cosharing_clusters` MCP tool for structured membership-to-cluster lookups, or query directly via SSH.
