---
name: skywatch-investigating-osprey-rules
description: Systematic investigation methodology for Osprey SML rules projects. Produces structured text reports on project structure, labels, models, UDFs, and execution graphs.
polytoken:
  tags: [skywatch]
---
...
Use `glob` and `file_read` to check for the required project components. Report each as
present or absent.
...
Use `file_read` to check if this file exists.
