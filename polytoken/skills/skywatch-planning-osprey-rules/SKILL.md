---
name: skywatch-planning-osprey-rules
...
1. **Investigator report** — structured text report produced by the `osprey-rule-writer`
   agent's investigation phase (using `skywatch-investigating-osprey-rules`),
...
- Chain to `skywatch-osprey-sml-reference` skill to look up common labeling patterns if you
...
**When to chain to `skywatch-osprey-sml-reference`:**
...
Load with the `skill` tool: `skywatch-osprey-sml-reference`
...
| "I know the type system" | No. SML type rules are strict. | Load `skywatch-osprey-sml-reference` if uncertain about EntityJson vs JsonData. |
...
**Output:** A confirmed rule specification in plain text. The `osprey-rule-writer`
agent uses this spec when loading `skywatch-authoring-osprey-rules` to implement
the SML, along with the investigator report.
