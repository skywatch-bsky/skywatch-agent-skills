...
**Tool Usage:**
- [ ] Used ClickHouse queries via SSH (data-analyst) to pull rule hits
- [ ] Used `mcp__skywatch-mcp__cosharing_clusters` with `did` to check co-sharing cluster membership
- [ ] Used `mcp__skywatch-mcp__domain_check` on discovered domains (if any)
...
**Tool Usage:**
- [ ] Used ClickHouse queries via SSH (data-analyst) for activity timeline and aggregation
- [ ] Used `mcp__skywatch-mcp__ip_lookup` on any extracted IP addresses
- [ ] Used `mcp__skywatch-mcp__whois_lookup` on discovered domains
...
**Tool Usage:**
- [ ] Used `mcp__skywatch-mcp__content_similarity` to find matching content across accounts
- [ ] Used ClickHouse queries via SSH (data-analyst) with GROUP BY to cluster by shared infrastructure
- [ ] Compared posting timestamps for tight synchronisation patterns
- [ ] Used `mcp__skywatch-mcp__cosharing_clusters` / `mcp__skywatch-mcp__cosharing_pairs` to check co-sharing relationships
- [ ] Used `mcp__skywatch-mcp__cosharing_evolution` to trace cluster history (if cluster found)
...
**Tool Usage:**
- [ ] Used ClickHouse queries via SSH (data-analyst) to aggregate engagement patterns
- [ ] Used `mcp__skywatch-mcp__url_expand` to resolve link destinations and targets
...
**Tool Usage:**
- [ ] Used ClickHouse queries via SSH (data-analyst) to extract rule statistics
...
- [ ] Reviewed `skywatch-reporting-results` skill for formatting requirements
