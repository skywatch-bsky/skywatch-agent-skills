...
description: >-
  Proactive network-wide threat scanning over a specified time window. Use when
  looking for emerging threats, incident upticks, anomalous network traffic,
  coordination patterns, or detection gaps.
...
- ClickHouse work is read-only. Dispatch ClickHouse query work to `data-analyst` subagents.
- Load `querying-clickhouse` for ClickHouse execution details when writing or reviewing query prompts.
...
3. Copypasta and similarity: use the content_similarity tool where appropriate to detect template reuse, highly similar posts, or repeated phrasing across the network.
...
Dispatch to `data-analyst`, using ClickHouse sidecar tables and read-only Skywatch MCP tools where useful:
...
6. MCP drill-down: when cluster IDs or DIDs warrant detail, use read-only tools such as cosharing_clusters, cosharing_pairs, and cosharing_evolution.
...
Coordination is not automatically inauthentic. If a cluster needs deeper narrative classification, use `classify-cluster` after the network scan identifies it as a follow-up target.
...
Use this BLIND-style shape. Load `reporting-results` when producing a formal report or when the user asks for fuller report conventions.
...
Ask the user before creating, editing, testing against production-like data, deploying, disabling, or tuning any Osprey rule. If the user approves rule work, load the appropriate follow-up skill from the osprey-rules plugin instead of embedding rule-authoring instructions here:
